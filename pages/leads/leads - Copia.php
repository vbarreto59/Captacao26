...
if ($fase_ativa) { $where .= " AND fase_funil = ?"; $params[] = $fase_ativa; }
...
<div class="container-fluid px-3 py-3">
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center mb-4 gap-3">
        <div>
            ...
            <p class="text-muted small mb-0"><?= count($lista) ?> registros encontrados</p>
        </div>
    ...
    <div class="mb-4">
        <div class="scroll-x mb-2">
            <a href="leads.php" class="btn btn-sm <?= (!$temp_ativa && !$fase_ativa) ? 'btn-dark' : 'btn-outline-secondary' ?> px-3">Todos</a>
            <a href="?temperatura=Quente<?= $fase_ativa ? '&fase='.$fase_ativa : '' ?>" class="btn btn-sm <?= $temp_ativa == 'Quente' ? 'btn-danger' : 'btn-outline-danger' ?> px-3">🔥 Quentes</a>
            <a href="?temperatura=Morno<?= $fase_ativa ? '&fase='.$fase_ativa : '' ?>" class="btn btn-sm <?= $temp_ativa == 'Morno' ? 'btn-warning' : 'btn-outline-warning' ?> px-3">⚖️ Mornos</a>
            <a href="?temperatura=Frio<?= $fase_ativa ? '&fase='.$fase_ativa : '' ?>" class="btn btn-sm <?= $temp_ativa == 'Frio' ? 'btn-info' : 'btn-outline-info' ?> px-3">❄️ Frios</a>
        </div>
        <div class="scroll-x">
                ...
                <table id="tabelaLeads" class="table align-middle mb-0">
                    ...
                    <tbody>
                        ...
                        <tr id="row-lead-<?= $l['id'] ?>">
                            ...
                            <td class="text-center"><span class="fw-bold text-success">R$ <?= number_format($l['valor_max'], 0, ',', '.') ?></span></td>

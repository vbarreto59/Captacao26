...
$busca = $_GET['busca'] ?? '';
$favorito_ativa = isset($_GET['favorito']) && $_GET['favorito'] == '1' ? true : false;
...
if ($favorito_ativa) { $where .= " AND favorito = 1"; }
...
<div class="container-fluid px-3 py-3">
    ...
    <div class="scroll-x mb-2">
        <?php foreach ($fases_lista as $f): ?>
            <a href="?fase=<?= urlencode($f) . ($temp_ativa ? "&temperatura=$temp_ativa" : "") . ($busca ? "&busca=$busca" : "") . ($favorito_ativa ? "&favorito=1" : "") ?>" 
               class="btn btn-sm <?= getFaseColor($f) ?> <?= ($fase_ativa == $f) ? 'active shadow border-dark' : 'border' ?> px-3" style="white-space: nowrap;"><?= $f ?></a>
        <?php endforeach; ?>
    </div>

    <!-- Filtros de Temperatura + Estrela + Limpar -->
    <div class="d-flex flex-wrap gap-2 mb-4 align-items-center filtros-container">
        ...
        <a href="?<?= $fase_ativa ? "fase=".urlencode($fase_ativa)."&" : "" ?><?= $busca ? "busca=$busca&" : "" ?><?= $favorito_ativa ? "favorito=1&" : "" ?>" class="filter-temp-link <?= $temp_ativa == '' ? 'active' : '' ?>">Todos</a>
        <?php foreach ($temps_lista as $key => $label): ?>
            <a href="?temperatura=<?= $key . ($fase_ativa ? "&fase=".urlencode($fase_ativa) : "") . ($busca ? "&busca=$busca" : "") . ($favorito_ativa ? "&favorito=1" : "") ?>" 
               class="filter-temp-link <?= $temp_ativa == $key ? 'active' : '' ?>"><?= $label ?></a>
        <?php endforeach; ?>
        
        <!-- Filtro Favoritos (Estrela) -->
        <a href="?favorito=1<?= $fase_ativa ? "&fase=".urlencode($fase_ativa) : "" ?><?= $temp_ativa ? "&temperatura=$temp_ativa" : "" ?><?= $busca ? "&busca=$busca" : "" ?>" 
           class="filter-temp-link <?= $favorito_ativa ? 'active' : '' ?>">
            ⭐ Favoritos
        </a>

        <?php if ($fase_ativa || $temp_ativa || $busca || $favorito_ativa): ?>
        ...
        <td>
            <div class="name-row-container d-flex align-items-center gap-2">
                <i class="bi <?= $l['favorito'] ? 'bi-star-fill text-warning' : 'bi-star text-muted' ?> btn-favorito fs-5" 
                   data-id="<?= $l['id'] ?>" style="cursor: pointer;"></i>
...
<!-- Scripts personalizados (as bibliotecas já estão no header.php) -->
<script>
$(document).ready(function() {
    ...
    $(document).on('change', '.select-step', function() {
        ...
        const step = $(this).val();
        $.post('leads.php', { action: 'update_step', id: id, step: step }, function(res) {
            if (res.status === 'success') {
                alert('Passo de atendimento atualizado!');
            } else {
    ...
    $('#formObs').on('submit', function(e) {
        ...
        $.post('leads.php', { action: 'update_obs', id: id, observacoes: obs }, function(res) {
            if (res.status === 'success') {
                $(`#obs-preview-${id}`).html(obs.replace(/\n/g, "<br />"));
                $(`#obs-preview-card-${id}`).html(obs.replace(/\n/g, "<br />"));
                $('#modalObs').modal('hide');
            }
        }, 'json');
    });
    ...
    $(document).on('click', '.temp-badge', function() {
        ...
        $.post('leads.php', { action: 'update_temp', id: id, temp: temp }, function(res) {
            if (res.status === 'success') {
                location.reload(); // Recarrega para aplicar as cores
            }
        }, 'json');
    });

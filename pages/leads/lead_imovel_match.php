...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['acao'])) {
    ...
    if ($leadId > 0) {
        try {
            ...
            elseif ($_POST['acao'] === 'atualizar_fase_funil') {
                $novoStatus = ($valor == 1) ? 'Perdido' : null;
                $sql = "UPDATE leads SET fase_funil = :status WHERE id = :id";
                $stmt = $pdo->prepare($sql);
                $sucesso = $stmt->execute(['status' => $novoStatus, 'id' => $leadId]);
                echo json_encode(['sucesso' => $sucesso]);
                exit();
            }
            elseif ($_POST['acao'] === 'atualizar_nome') {
                $novoNome = trim($_POST['valor_texto'] ?? '');
                if (empty($novoNome)) {
                    echo json_encode(['sucesso' => false, 'erro' => 'Nome não pode ficar vazio']);
                    exit();
                }
                $sql = "UPDATE leads SET nome = :nome WHERE id = :id";
                $stmt = $pdo->prepare($sql);
                $sucesso = $stmt->execute(['nome' => $novoNome, 'id' => $leadId]);
                echo json_encode(['sucesso' => $sucesso, 'novo_nome' => $novoNome]);
                exit();
            }
        } catch (Exception $e) {
...
// ============================================================
if ($_SERVER['REQUEST_METHOD'] === 'POST' && (!isset($_POST['acao']) || !in_array($_POST['acao'], ['atualizar_contatar_hoje', 'atualizar_compartilhado_parceiro', 'atualizar_favorito', 'atualizar_fase_funil', 'atualizar_nome']))) {
    $acao = $_POST['acao'] ?? '';
    $leadId = intval($_POST['lead_id'] ?? 0);
...
<head>
    ...
    <style>
        * { box-sizing: border-box; }
        ...
        .btn-edit-nome:hover, .btn-edit-nome:active {
            background: rgba(255,255,255,0.2);
        ...
        .edit-nome-buttons button:hover {
    ...
    <script>
        ...
        // Função para marcar lead como Perdido
        function marcarPerdido(checkbox, leadId) {
            if (!checkbox.checked) {
                if (confirm('Deseja reativar este lead? Ele voltará a aparecer na lista.')) {
                    const formData = new FormData();
                    formData.append('acao', 'atualizar_fase_funil');
                    formData.append('lead_id', leadId);
            ...
            if (confirm('Marcar este lead como PERDIDO? Ele será removido da lista de ativos.')) {
                const card = document.getElementById('card-lead-' + leadId);
                ...
                .then(data => {
                    if (data.sucesso) {
                        ...
                        if (card) card.classList.remove('removendo');
        ...
        // ========== FUNÇÕES PARA EDITAR NOME DO LEAD ==========
        function iniciarEdicaoNome(leadId, nomeAtual) {
            pararPropagacao(event);
            const container = document.getElementById('lead-nome-container-' + leadId);
            if (!container) return;
            
            // Cria elementos de edição
            const input = document.createElement('input');
            input.type = 'text';
            input.value = nomeAtual;
            input.className = 'edit-nome-input';
            ...
            btnCancelar.onclick = (e) => {
                e.stopPropagation();
                restaurarExibicaoNome(leadId, nomeAtual);
            };
            
            const divButtons = document.createElement('div');
            divButtons.className = 'edit-nome-buttons';
            divButtons.appendChild(btnSalvar);
            divButtons.appendChild(btnCancelar);
            
            // Limpa o container e adiciona os novos elementos
            container.innerHTML = '';
        ...
        function salvarNome(leadId, novoNome) {
            ...
            const formData = new FormData();
            formData.append('acao', 'atualizar_nome');
            formData.append('lead_id', leadId);
            formData.append('valor_texto', novoNome);
            
            fetch(window.location.href, { method: 'POST', body: formData })
            .then(response => response.json())
            .then(data => {
                if (data.sucesso) {
                    restaurarExibicaoNome(leadId, data.novo_nome);
                } else {
                    alert('Erro ao salvar: ' + (data.erro || 'Tente novamente'));
                    // Restaura o nome original
                    const nomeOriginal = document.getElementById('lead-nome-container-' + leadId).getAttribute('data-nome-original');
                    restaurarExibicaoNome(leadId, nomeOriginal);
                }
            })
            .catch(error => {
                console.error('Erro:', error);
                alert('Erro de comunicação ao salvar o nome.');
                const nomeOriginal = document.getElementById('lead-nome-container-' + leadId).getAttribute('data-nome-original');
                restaurarExibicaoNome(leadId, nomeOriginal);
            });
        }
        
        function restaurarExibicaoNome(leadId, nome) {
            const container = document.getElementById('lead-nome-container-' + leadId);
            if (!container) return;
            
            // Remove atributo de nome original se existir
            container.removeAttribute('data-nome-original');
            
            // Recria a estrutura original
            const spanTexto = document.createElement('span');
            spanTexto.className = 'lead-nome-texto';
            spanTexto.textContent = nome;
            spanTexto.onclick = (e) => {
                e.stopPropagation();
                iniciarEdicaoNome(leadId, nome);
            };
            
            const btnEditar = document.createElement('button');
            btnEditar.className = 'btn-edit-nome';
            ...
            // Atualiza também o nome no atributo de dados para possível uso futuro
            container.setAttribute('data-nome-original', nome);
        }
    </script>
</head>
<body>
<div class="container">
    ...
    <?php else: ?>
        <?php foreach ($leads as $lead): 
            ...
            $nomeLead = htmlspecialchars($lead['nome'] ?: $lead['primeiro_nome'] ?: 'Sem nome');
        ?>
        <div class="lead-card <?= $classeDestaqueHoje ?>" id="card-lead-<?= $lead['id'] ?>">
            <div class="lead-header" onclick="toggleImoveis(<?= $lead['id'] ?>)">
                <div class="lead-title">
                    <!-- Container para edição do nome -->
                    <div id="lead-nome-container-<?= $lead['id'] ?>" class="lead-nome-container" data-nome-original="<?= $nomeLead ?>">
                        <span class="lead-nome-texto" onclick="pararPropagacao(event); iniciarEdicaoNome(<?= $lead['id'] ?>, '<?= addslashes($nomeLead) ?>')">
                            <?= $nomeLead ?>
                        </span>
                        <button class="btn-edit-nome" onclick="pararPropagacao(event); iniciarEdicaoNome(<?= $lead['id'] ?>, '<?= addslashes($nomeLead) ?>')" title="Editar nome">
                            ✏️
                        </button>
                    </div>
                ...
                <div class="lead-header-actions">
                    ...
                    <label class="toggle-label" onclick="pararPropagacao(event);">
                        <input type="checkbox" onchange="marcarPerdido(this, <?= $lead['id'] ?>)"> 
                        ❌ Perdido
                    </label>

<?php
// listagem_obs_parceiros.php - Listagem com DataTables para ordenação e filtros
session_start();
...
$sql = "SELECT id, nome, primeiro_nome, valor_max, obs_parceiros, compartilhado_parceiro, favorito, contatar_hoje, deleted_at 
...
<head>
    ...
    <style>
        ...
        /* Responsividade: overflow horizontal em telas pequenas */
...
<div class="container mt-4 mb-5">
                ...
                <table id="tabelaLeads" class="table table-hover align-middle mb-0 w-100">
                    <thead class="table-light">
                        <tr>
                            ...
                            <th style="width: 120px;">Valor Máximo</th>
                            <th style="width: 60px;"><i class="bi bi-star-fill"></i></th>
                            <th>Nome Completo</th>
                            <th>Primeiro Nome</th>
                            <th>Obs Parceiros</th>
                    ...
                    <tbody>
                        <?php foreach ($leads as $index => $lead): 
                            ...
                            $valor_max = ($lead['valor_max'] ?? 0) > 0 ? 'R$ ' . number_format($lead['valor_max'], 0, ',', '.') : '—';
                        ?>
                        <tr id="row-lead-<?= $lead['id'] ?>" class="<?= $row_class ?>">
                            <td data-label="#"><?= $index + 1 ?></td>
                            <td data-label="Valor Máximo" class="fw-semibold"><?= $valor_max ?></td>
    ...
    <div class="text-muted small mt-3 text-center">
        ...
        <i class="bi bi-arrow-down-up"></i> Clique nos cabeçalhos das colunas para ordenar.
    </div>
</div>

<!-- Modais Editar Nome Completo -->
...
$(document).ready(function() {
    ...
    var table = $('#tabelaLeads').DataTable({
        ...
        "order": [[0, 'asc']], // Ordenar pela coluna # (primeira coluna)
        "columnDefs": [
            { "orderable": true, "targets": [0, 1, 2, 3, 4, 5, 6, 7, 8] },
            { "searchable": true, "targets": [0, 1, 2, 3, 4, 5] }, // somente colunas de texto podem ser buscadas
            { "type": "num", "targets": 0 }  // tratar coluna # como número
        ],
        "pageLength": 100,
        "responsive": false, // não usar plugin responsive para evitar conflitos com estilos personalizados
        "drawCallback": function() {
            // Reaplicar classes de estilo após redesenho do DataTables
            $('tbody tr').each(function() {
                var rowId = $(this).attr('id');
                if (rowId) {
                    var originalRow = $('#' + rowId);
                    if (originalRow.hasClass('row-shared')) $(this).addClass('row-shared');
                    if (originalRow.hasClass('row-deleted')) $(this).addClass('row-deleted');
                }
            });
        }
    });
    ...
    $('#btnSalvarNomeCompleto').on('click', function() {
        ...
        $.post(currentScript, {
            ...
            if (res.status === 'success') {
                ...
                // Recarregar DataTables para atualizar ordenação

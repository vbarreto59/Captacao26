...
// FILTROS
// ================================================
$localizacao = trim($_GET['localizacao'] ?? ''); // Agora é o bairro selecionado
...
// Novo filtro de localização: comparação exata com o bairro
...
// Buscar lista de bairros distintos para o select
...
<style>
    ...
    .carac-item i { width: 20px; color: #0d6efd; }
    .comodidades-badge {
        display: flex;
        flex-wrap: wrap;
        gap: 6px;
    ...
    .valores-detalhe .preco-principal {
        font-size: 1.25rem;
        font-weight: 800;
        color: #0d6efd;
        margin-bottom: 5px;
    }
    .valores-detalhe .taxas-linha {
        display: flex;
        flex-wrap: wrap;
        gap: 12px;
        font-size: 0.75rem;
    ...
    .aceita-text { font-size: 0.7rem; color: #28a745; font-weight: bold; }
    .filtros-card {
        background: white;
        border-radius: 16px;
        padding: 20px;
    ...
    .btn-limpar:hover { background-color: #5a6268; color: white; }
    .faixa-preco-ajuda { font-size: 0.7rem; color: #6c757d; margin-top: 4px; }
    .corretor-titular {
        font-size: 0.8rem;
        color: #0d6efd;
        background: #e7f1ff;
            ...
            <div class="col-md-3">
                ...
                <select name="localizacao" class="form-select">
                    ...
                    <?php foreach ($bairros as $bairro): ?>
                        <option value="<?= htmlspecialchars($bairro) ?>" <?= $localizacao == $bairro ? 'selected' : '' ?>>
                ...
                <div class="card-body d-flex flex-column">
                    ...
                    <div class="valores-detalhe">
                        <div class="preco-principal">
                            R$ <?= number_format($im['preco'], 2, ',', '.') ?>
                        </div>
                        <?php if($im['valor_condominio'] > 0 || $im['valor_iptu'] > 0 || $im['valor_sinal'] > 0 || $im['agua_inclusa_condominio'] == 1 || $im['gas_incluso_condominio'] == 1): ?>
                            <div class="taxas-linha">
                                ...
                                <?php if($im['agua_inclusa_condominio'] == 1): ?>
                                    <span class="destaque-incluso"><i class="bi bi-droplet-fill"></i> Água Inc.</span>
                                <?php endif; ?>
                                <?php if($im['gas_incluso_condominio'] == 1): ?>
                                    <span class="destaque-incluso"><i class="bi bi-fire"></i> Gás Inc.</span>
                                <?php endif; ?>
